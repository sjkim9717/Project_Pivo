...
public class Test : MonoBehaviour {
    ...
    private void ChangeSelectObjectLayer(Vector3 start, Vector3 finish) {
        ...
        Vector3 center = new Vector3(playerRigidbody.position.x, playerRigidbody.position.y, (finish.z + start.z)*0.5f);

        Vector3 halfExtents = new Vector3(10, 20, Mathf.Abs(finish.z - start.z / 2)); // 직사각형의 절반 크기

        Vector3 direction = (finish.z - start.z) > 0 ? transform.forward : -transform.forward;

        RaycastHit[] hits = Physics.BoxCastAll(center, halfExtents, direction);
